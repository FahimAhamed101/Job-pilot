...
import { uploadLibraryItem, updateLibraryItem } from './fileUpload';
...
const Library = () => {
    ...
    // API hooks - only use queries and delete mutation
    const { data: libraryItems = [], isLoading, error, refetch } = useGetAllLibraryItemsQuery();
    ...
    const getFileUrl = (url) => {
        ...
        return `https://unirenic-twittery-cammie.ngrok-free.dev${url}`;
    };
    ...
    // FIXED: Using custom upload function
    const handleOk = async () => {
        try {
            ...
            try {
                // Use custom upload function instead of RTK Query
                const result = await uploadLibraryItem(formData);
                ...
                if (uploadError.message.includes('Unexpected token') || uploadError.message.includes('JSON')) {
                    ...
                    message.error(uploadError.message || 'Upload failed. Please try again.');
                }
            }
        } catch (formError) {
    ...
    const handleMainFileUpload = (file) => {
        ...
        if (selectedFileType) {
            if (selectedFileType === 'video' && !file.type.startsWith('video/')) {
                message.error('Please select a video file for video type');
                return false;
            }
            ...
            if (selectedFileType === 'text' && !file.type.startsWith('text/') && file.type !== 'application/msword' && file.type !== 'application/vnd.openxmlformats-officedocument.wordprocessingml.document') {
                message.error('Please select a text document for text type');
                return false;
            }
        }
    ...
    const handleThumbnailUpload = (file) => {
        ...
        if (!file.type.startsWith('image/')) {
            message.error('Please select an image file for thumbnail');
            return false;
        }
    ...
    // FIXED: Update handler using custom function
    const handleEditOk = async () => {
        try {
            ...
            console.log('Form data:', {
                ...
                hasNewFile: !!mainFile,
                hasNewThumbnail: !!thumbnailFile
            });

            setIsUpdating(true);

            try {
                await updateLibraryItem(selectedItem._id, formData);
                ...
                if (updateError.message.includes('Unexpected token') || updateError.message.includes('JSON')) {
                    message.error('Server configuration error: Contact administrator to fix backend JSON parsing.');
                } else {
                    message.error(updateError.message || 'Failed to update library item');
                }
            }
        } catch (error) {
                            ...
                            <div className="border-2 border-dashed border-gray-300 rounded-lg p-4">
                                ...
                                {mainFilePreview ? (
                                    ...
                                    <Upload
                                        ...
                                        accept=".pdf,.mp4,.mov,.avi,.txt,.doc,.docx,.jpg,.jpeg,.png"
                                    >
                                        <div className="text-center cursor-pointer py-8">
                                            ...
                                            <p className="text-gray-500 text-sm mt-1">PDF, Video, Text, or Image files</p>
                                        </div>
                                    </Upload>
                                )}
                            </div>
